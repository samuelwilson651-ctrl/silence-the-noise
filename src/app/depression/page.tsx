...
const articles = [
  { href: "/articles/when-nothing-seems-to-work", title: "When Nothing Seems to Work", desc: "For the reader who has tried everything and is still in the dark. Healing is rarely linear — and slow progress is still progress.", pillar: "Pillar V: Renewal Under Pressure", tag: "Featured" },
];

const coming = [
  "The Cliff and the Gorge",
  "Love as Medicine",
  "The Missing Elements of Purpose",
  "Depression and Human Connection",
  "When the Soul Becomes Tired",
  "Rediscovering Hope",
];
    ...
    <div style={{ background: "#faf7f2" }}>
      ...
      <section style={{ padding: "5rem 2rem" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          {articles.map((a) => (
            <Link key={a.href} href={a.href} style={{ display: "block", background: "white", border: "1px solid #e4ddd2", borderRadius: 2, padding: "2.25rem", marginBottom: "1rem", position: "relative" }}>
              {a.tag && (
                <span style={{ position: "absolute", top: -1, left: "2rem", background: "#c9a84c", color: "#0f1e35", fontFamily: "var(--font-inter,sans-serif)", fontSize: "0.6rem", letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 700, padding: "0.2rem 0.65rem", borderRadius: "0 0 3px 3px" }}>{a.tag}</span>
              )}
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "0.75rem", marginTop: a.tag ? "0.5rem" : 0 }}>
          ...
          {/* Coming soon */}
          <p style={{ fontFamily: "var(--font-inter,sans-serif)", fontSize: "0.68rem", letterSpacing: "0.22em", textTransform: "uppercase", color: "#c9a84c", margin: "3rem 0 1.5rem" }}>Coming Soon</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: "1rem" }}>
            {coming.map((title) => (
              <div key={title} style={{ background: "white", border: "1px solid #e4ddd2", borderRadius: 2, padding: "1.5rem", opacity: 0.6 }}>

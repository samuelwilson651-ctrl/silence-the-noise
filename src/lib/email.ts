...
function getFrom() {
  return `${process.env.RESEND_FROM_NAME ?? "Dr. Samuel E. Wilson MD"} <${process.env.RESEND_FROM_EMAIL ?? "noreply@samuelewilson.org"}>`;
}
...
export async function sendConsultationConfirmation(email: string, firstName: string) {
  ...
  await resend.emails.send({
    ...
    html: wrap(`<h2>Confirmed, ${firstName}.</h2><div class="rule"></div><p>Dr. Wilson will reach out within 2 business days to schedule your session.</p><a href="https://samuelewilson.org/dashboard" class="cta">View Dashboard →</a>`),
  });
}

export async function sendSpeakingInquiry(data: { name: string; email: string; organization: string; message: string }) {
  const resend = getResend();
  const fromAddr = process.env.RESEND_FROM_EMAIL ?? "noreply@samuelewilson.org";
  await resend.emails.send({
    from: getFrom(), to: fromAddr,
